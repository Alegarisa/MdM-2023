...
```{r}
d <- import(here("data", "principals_co1_co2_w1.sav"), setclass = "tbl_df") # 30 principals
...
```{r}
# age
mean(d1$demo002_4, na.rm = TRUE)
sd(d1$demo002_4, na.rm = TRUE)
...
mean(d1$item001_4[!d1$item001_4 %in% c(69)], na.rm = TRUE) #
sd(d1$item001_4[!d1$item001_4 %in% c(69)], na.rm = TRUE)
range(d1$item001_4[!d1$item001_4 %in% c(69)], na.rm = TRUE) # range 69 - 1050

d1 %>%
  filter(item001_4 == "69") # seems too low for a primary school with grades 1-6. 

d1 %>% filter(item013_4 == "3") # checking if one of the 2 rural schools is the one with 69 students total. R/: No


# number of teachers in school
mean(d1$item011_4, na.rm = TRUE) #
sd(d1$item011_4, na.rm = TRUE)
range(d1$item011_4, na.rm = TRUE)
