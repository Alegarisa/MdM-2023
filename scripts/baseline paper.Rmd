...
```{r}
# raw_par_w1c1 <- read.spss(here("nopublish", "01. SPSS Caregivers Assessment 1 Cohort 1.ESP.sav"),
#                           to.data.frame = TRUE) # this dataset is the upload that still has missing IDs. It only shows the value labels, but not the values. QUANT analyses can't be completed. I am unsure if this is due to foreign or for some other reason. 
```
...
raw_par_w1c1 <- read.spss(here("nopublish", "01_SPSS Caregivers Assessment 1 Cohort 1_ESP (1).sav"),
                          to.data.frame = TRUE)
...
raw_par_w1c2 <- read.spss(here("nopublish", "01_SPSS Caregivers Assessment 1 Cohort 2_ESP (1).sav"),
                          to.data.frame = TRUE)
...
```{r}
par_w1 %>%
  haven::write_sav(here("nopublish", "par_all_w1.sav"))
```
